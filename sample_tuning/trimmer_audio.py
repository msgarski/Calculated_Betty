...
for nazwa in file_list:                                     # todo policzyć jeszcze noise level
    ...
    if os.path.isfile(abs_path):                            # if element of list is real file...do something:
        ...
        print(" noise level from file: ", noise_level)
        ...
        word = padding_audio(data, 2, frame_rate)
        word_size = len(word)
        print(" dlugosc po: ", word_size)
        ...
        try:
            if word_size < time_to_frames(0.5, frame_rate):  # if word is shorter than 0.5 sec ->
                try:                                         # -> this is not good effect...save it!!!
        ...
        if word_size >= time_to_frames(0.3, 44100):
            indeks += 1
            # name_of_class = classname_extractor(nazwa)
            # print("nazwa: ", nazwa, "rozmiar: ", word_size, "start i end: ",result, " nazwa klasy: ", name_of_class)
            filename = os.path.join(path_save, nazwa)  # new path for cutted words
            write(filename, frame_rate, word)
